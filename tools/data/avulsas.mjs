...
//  · o pacote de clã da Senda Mandaloriana — arquétipo cross-class que substitui a
...
const COMO_ADICIONAR =
  ...
  "<p><em>Atalho:</em> o compêndio de Classes já traz variantes prontas (ex.: <em>Guardião (Soresu)</em>, <em>Guerreiro de Clã</em>) com estas habilidades embutidas — basta arrastar a variante e pular este passo.</p>";
...
const AVULSAS = [
  ...
  // ── Senda Mandaloriana: o pacote de clã (substitui a especialização) ──────
  {
    ...
    desc: "<p>A Senda <strong>substitui a especialização normal</strong> da classe (esse é o custo principal, igual para todos) e o personagem passa a evoluir pela coluna <strong>XP Especial</strong>. Por cima disso, cada classe faz um sacrifício equivalente:</p><ul>" +
      "<li><strong>Guerreiro de Clã</strong> <em>(Veterano)</em>: <em>Pilotar</em>, <em>Desarmar e Subjugar</em> e o <em>Dano Crítico</em> param de progredir, e o Resol&rsquo;nare pede <strong>cinco</strong> votos em vez de três. Mantém os <em>Ataques Múltiplos</em> inteiros.</li>" +
      "<li><strong>Caçador Solitário</strong> <em>(Operativo)</em>: o <em>Ataque Furtivo</em> e a <em>Percepção</em> param de progredir; em troca, veste Beskar, usa jetpack e ganha <strong>Rastrear</strong> no lugar de <em>Escalar</em>.</li>" +
      "<li><strong>Armeiro</strong> <em>(Técnico)</em>: perde o <em>Desconto Tecnológico</em> e <em>Desativar Droides</em> — ele forja, não sabota. É a única especialização do Técnico que <strong>mantém</strong> <em>Operar e Consertar Máquinas</em>.</li>" +
      "<li><strong>Herege de Armadura</strong> <em>(Sensível à Força)</em>: Foco calculado como o de um Sensível de <strong>−1 nível</strong> e <strong>Teto de Grandeza na 6ª</strong>; em troca, o clã lhe ensina <strong>uma Forma de Sabre</strong> (até a técnica do 6º nível).</li>" +
      "</ul><p>As quatro fichas estão prontas no compêndio de Classes, com o pacote de clã já embutido — <strong>não é preciso somar nada</strong>. <em>Portador da Herança</em> não é uma quinta ficha: é quem carrega o Sabre Sombrio, e vale para qualquer uma das quatro.</p>",
  },
  {
